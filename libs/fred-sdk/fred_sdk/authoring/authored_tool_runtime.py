...
class _AuthorRuntime:
    """
    Private runtime bundle for Python-authored tool handlers.

    Why this exists:
    - authored tool handlers need access to the bound model, the fallback
      tool invoker, artifact/resource ports, and media fetch
    - these are runtime details, not part of the public authoring API

    How to use it:
    - created once per bind_handlers call inside the toolset registration
    - injected into ToolContext so authored tools reach Fred capabilities
    ...
    async def fetch_media(self, document_uid: str, file_name: str) -> bytes:
        if self._media_client is None:
            if self.ports.media_fetcher is None:
                raise RuntimeError(
                    "Authored tools require a media_fetcher port to fetch packaged media."
                )
            self._media_client = self.ports.media_fetcher
        return await self._media_client(document_uid, file_name)
