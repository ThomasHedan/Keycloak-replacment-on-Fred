...
class GraphWorkflow:
    ...
    error_routes: Mapping[str, str] = field(default_factory=dict)
    """
    Declare per-node fallback targets for unhandled exceptions.

    Key is a node id.  Value is the node to route to when that handler raises.
    When a node raises and an entry exists here, the runtime catches the exception,
    writes ``node_error`` into graph state, emits a ``NodeErrorRuntimeEvent``, and
    continues execution at the declared fallback node instead of crashing.
        ...
        return GraphDefinition(
            ...
            nodes=tuple(
                GraphNodeDefinition(
                    ...
                    on_error=self.error_routes.get(node_id),
                )
